...
>
> 📍 另见 [ROADMAP.md](ROADMAP.md)：三阶段演进路线（低代码验证 → Python 原型 → Java 生产化）。本文档是其中**阶段二**的详细技术计划。
...
| 音频理解 | **通义 Qwen3.5-Omni**（DashScope OpenAI 兼容接口，已确认；2026-08-15 实测模型名改为 **qwen3.5-omni-flash**，备选 qwen3.5-omni-plus） | 无 CC/AI 字幕时启用：音频直接生成大纲/问题，**不做转写** |
